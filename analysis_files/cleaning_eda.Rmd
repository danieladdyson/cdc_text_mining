...
3) isolating the resulting conditions

The word count alone is fairly indicative of the major injury categories.
```{r word count distribution by outcome, echo=TRUE}
dat[,word_cnt:=sapply(strsplit(text, " "), length)]
ggplot(data=dat, aes(x=CASE_CODE_TITLE1, y = word_cnt)) + 
  geom_boxplot() + 
  coord_flip() + 
  theme_bw() + 
  labs(y = "Word count", x = "Case Code")
```


The text is, by & large, fairly garbage. Numerous misspellings of e.g. laceration
...
all_stops <- stopwords("en")) # set stopwords
txt2 <- removeWords(txt2, all_stops) # remove stopwords

dx <- sapply(strsplit(x = txt2, split = "dx"), "[", 2)
txt2 <- gsub("dx|d x|yom|ym|yof|yf|yo|yom, ym, y, yom, yof, yo m, yo f, y of, y om, y o f, y o m,ym, yf","",txt2)

txt3 <- txt2 %>% unnest_tokens(word, text) %>% group_by(word) %>% count() 
setorder(txt3, -n)

txt4 <- cbind(txt3, txt_stem)

write.csv(txt4, file = "word_count_with_stem.csv", row.names = FALSE)
...
dat_trn <- dat[trn_tst=="train",]
tok_fun <-  word_tokenizer
it_train <- itoken(dat_trn$text, 
...
nfold <- 10
word_trn <- cv.glmnet(x = dtm_train_tfidf, y = dat_trn[['event1']], 
                              family = 'multinomial', 
                              alpha = 1,
                              type.measure = "auc",
                              nfolds = nfold,
